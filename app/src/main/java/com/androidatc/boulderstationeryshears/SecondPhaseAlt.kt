/*
This activity is called if text buttons is enabled. This activity is where player one will select
...
import com.androidatc.boulderstationeryshears.databinding.ActivitySecondPhaseAltBinding
import java.util.*

class SecondPhaseAlt : AppCompatActivity() {

    private lateinit var binding: ActivitySecondPhaseAltBinding
    // store player moves
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = ActivitySecondPhaseAltBinding.inflate(layoutInflater)
        setContentView(binding.root)

        // create instance of shared preferences
        sharedPreferences = getSharedPreferences("MyPrefs", Context.MODE_PRIVATE)

        // will determine if player 2 is human or computer with intent passed from previous activity
    ...
    fun determineOpponent() {
        ...
        if (humanOpponent) {
            val intent = Intent(this, ThirdPhaseAlt::class.java)
